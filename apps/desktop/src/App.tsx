...
export function App({
  ...
  const workspaceQuery = useQuery<WorkspacePayload>({
    ...
    refetchInterval: (query) => view.kind === 'processing' && task.active && !query.state.error ? 1000 : false,
    retry: false,
  });
  ...
  if (view.kind === 'booting') {
    ...
    content = <section><div className="print-hidden"><h2>报告</h2><p>当前报告为实时草稿，尚非锁定版本；下方可以明确保存不可变的草稿版本。</p></div>
    ...
    content = workspaceQuery.data ? <MaterialWorkspace payload={workspaceQuery.data}
      ...
      processDisabled={task.locked || task.canResume} processing={task.active}
